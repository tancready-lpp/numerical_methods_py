...
"""
# L = 1e2 -> M  = -1.05997 circa 2s
# L = 2e2 -> M  = -1.05986 circa 15s
# L = 3e2 -> M  = -1.05984 circa 90s
...
V = [((-1)**(i+j+k))/math.sqrt(i**2 + j**2 + k**2) for i in range(-L,L+1)
     for j in range(-L,L+1) for k in range(-L,L+1) if (i, j, k) != (0, 0, 0)]
print(len(V))
